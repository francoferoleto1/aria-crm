...
function useVoice({ onTranscript, onError }) {
  ...
  const start = useCallback(async () => {
    ...
    catch { setS("error"); onError("Permiso de micrófono denegado."); setTimeout(()=>setS("idle"),4000); return; }
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async (e) => {
    ...
    // onLogin se dispara via onAuthStateChange en el padre
  };

  const fillDemo = (role) => {
    setEmail(role==="admin" ? "admin@aria-demo.com" : "vendedor@aria-demo.com");
    setPassword("aria2024");
    setError("");
  };

  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#EFF6FF 0%,#F0F4FF 50%,#FAF5FF 100%)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:420}}>
        ...
        <div style={{background:"#fff",borderRadius:18,border:"1px solid #E2E8F0",padding:"32px 32px 28px",boxShadow:"0 4px 24px rgba(0,0,0,.07)"}}>
          ...
          <form onSubmit={handleLogin} style={{display:"flex",flexDirection:"column",gap:14}}>
            <div>
              <label style={{display:"block",fontSize:11.5,fontWeight:600,color:"#374151",marginBottom:5,textTransform:"uppercase",letterSpacing:".06em"}}>Email</label>
              <input
                type="email" value={email} onChange={e=>{setEmail(e.target.value);setError("");}}
                placeholder="tu@email.com"
                style={{width:"100%",padding:"10px 13px",border:"1.5px solid #E2E8F0",borderRadius:9,fontSize:13,color:"#0F172A",fontFamily:"inherit",outline:"none",transition:"border-color .2s",background:"#F8FAFC"}}
            ...
            <div>
              ...
              <div style={{position:"relative"}}>
                <input
                  ...
                  placeholder="••••••••"
                  style={{width:"100%",padding:"10px 40px 10px 13px",border:"1.5px solid #E2E8F0",borderRadius:9,fontSize:13,color:"#0F172A",fontFamily:"inherit",outline:"none",transition:"border-color .2s",background:"#F8FAFC"}}
                  onFocus={e=>e.target.style.borderColor="#93C5FD"}
                  onBlur={e=>e.target.style.borderColor="#E2E8F0"}
                />
            ...
            {error && (
              <div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:8,padding:"8px 12px",fontSize:11.5,color:"#B91C1C",fontWeight:500}}>
                {error}
              </div>
            )}

            <button type="submit" disabled={loading}
              style={{padding:"11px",background:"linear-gradient(135deg,#2563EB,#7C3AED)",color:"#fff",border:"none",borderRadius:10,fontFamily:"inherit",fontWeight:700,fontSize:13.5,cursor:loading?"wait":"pointer",opacity:loading?.7:1,transition:"opacity .2s",marginTop:2}}>
          ...
          <div style={{marginTop:22,paddingTop:18,borderTop:"1px solid #F1F5F9"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                {role:"admin",   label:"Admin",    sub:"Vista completa",  icon:"👑"},
                {role:"vendedor",label:"Vendedor", sub:"Vista comercial", icon:"💼"},
        ...
        <div style={{textAlign:"center",marginTop:16,fontSize:11,color:"#CBD5E1"}}>
          ARIA · Demo v1.0 · Datos de prueba
        </div>
      </div>
    </div>
  );
...
function Dashboard({ user, onLogout }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setContacts((ct||[]).map(dbToContact));
      setActivity(ac||[]);
  ...
  const confirm = async () => {
    ...
    if (contactId) {
      ...
      const {error:ue} = await supabase.from("contacts").update(dbUpdate).eq("id",contactId);
      ...
      const {data:ld} = await supabase.from("activity_log").insert({
        contact_id:contactId, contact_name:contactName, summary, changes:dbUpdate,
      }).select().single();
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#F0F4FF",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",overflow:"hidden"}}>
      ...
      <nav style={{height:54,background:"#fff",borderBottom:"1px solid #E2E8F0",display:"flex",alignItems:"center",padding:"0 22px",gap:10,flexShrink:0,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
        ...
        {/* User badge */}
        <div style={{display:"flex",alignItems:"center",gap:7,background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:20,padding:"4px 12px 4px 6px"}}>
          <div style={{width:22,height:22,borderRadius:"50%",background:isAdmin?"linear-gradient(135deg,#2563EB,#7C3AED)":"linear-gradient(135deg,#10B981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#fff"}}>
            {user.email[0].toUpperCase()}
          </div>
          <span style={{fontSize:11,color:"#334155",fontWeight:600}}>{isAdmin?"Admin":"Vendedor"}</span>
          {isAdmin && <span style={{fontSize:9,background:"#EEF2FF",color:"#4338CA",padding:"1px 6px",borderRadius:4,fontWeight:700}}>ADMIN</span>}
        </div>
        ...
        <button onClick={onLogout}
          style={{marginLeft:4,padding:"5px 12px",background:"transparent",border:"1px solid #E2E8F0",borderRadius:8,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:11,color:"#94A3B8",cursor:"pointer",fontWeight:500,transition:"all .15s"}}
...
export default function App() {
  ...
  if (session === undefined) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F0F4FF",fontFamily:"system-ui"}}>
      <div style={{fontSize:13,color:"#94A3B8"}}>Iniciando...</div>
    </div>
  );
